...
(assert (deep= @[1] (run-with-input [3 9 8 9 10 9 4 9 99 -1 8] [8])) "equal to 8")
(assert (deep= @[0] (run-with-input [3 9 8 9 10 9 4 9 99 -1 8] [-1])) "not equal to 8")
(assert (deep= @[0] (run-with-input [3 12 6 12 15 1 13 14 13 4 13 99 -1 0 1 9] [0])) "zero is zero")
(assert (deep= @[1] (run-with-input [3 12 6 12 15 1 13 14 13 4 13 99 -1 0 1 9] [22])) "nonzero is 1")
(assert (deep= @[0] (run-with-input [3 3 1105 -1 9 1101 0 0 12 4 12 99 1] [0])) "zero is zero immed")
(assert (deep= @[1] (run-with-input [3 3 1105 -1 9 1101 0 0 12 4 12 99 1] [-1])) "nonzero is 1 immed")
...
(assert (deep= @[999] (run-with-input prog-big [7])) "big below 8")
(assert (deep= @[1000] (run-with-input prog-big [8])) "big equal 8")
(assert (deep= @[1001] (run-with-input prog-big [9393])) "big gt 8")

# tests for aoc 9 (adding relative mode and long numbers)
(let [p @[109 1 204 -1 1001 100 1 100 1008 100 16 101 1006 101 0 99]]
  (assert (deep= (run-with-input p []) p)))
